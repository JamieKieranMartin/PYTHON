...
def Yahtzee():
    ...
    x = 0
    a = 0
    while keepdie != [5,5,5,5,5]:
        for roll in range(5-a):
            diceroll = randint(1,6)
            rollstogether.append(diceroll)
            if diceroll == 5:
                a += 1
                keepdie.append(diceroll)
                print("5!")
        print(rollstogether)
        rollstogether = []
        x += 1
        if keepdie == [5,5,5,5,5]:
            print("Yahtzee!" + " " + "You rolled:" + str(keepdie))
            print(x)
            exit()
